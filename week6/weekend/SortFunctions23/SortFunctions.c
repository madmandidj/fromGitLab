...
static void InsertionSortWithGap(Vector* _vec, ShouldSwap _shouldSwapFunc, int _numOfItems, int _gapMode)
{
	...
	while(1 <= gap)
	{
		// for (vecIndex = 0; vecIndex < _numOfItems; vecIndex += gap)
		// {
		// VectorGet(_vec, vecIndex + 1, &vecValL);
		// curIndex = vecIndex;
		curIndex = 1;
		while (curIndex < _numOfItems)
		{
			VectorGet(_vec, curIndex , &vecValL);
			VectorGet(_vec, curIndex + gap , &vecValR);
			needSwap = _shouldSwapFunc(vecValL, vecValR);
			if (needSwap)
			{
				curValR = vecValR;
				curValL = vecValL;
				for (backCurInd = curIndex; backCurInd >= 1; NULL)
				{
					SwapVecValsInsertion(_vec, backCurInd - gap, backCurInd , curValL, curValR);
					if (backCurInd - gap >= 1)
					{
						backCurInd -= gap;												
