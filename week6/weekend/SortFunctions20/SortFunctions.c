...
static int BubbleSwap(Vector* _v, int _index1, int _index2, int _swapVal1, int _swapVal2)
{
	/* index + 1 because Vector implementation starts from index = 1, not index = 0 */
	VectorSet(_v, _index2, _swapVal1);
	VectorSet(_v, _index1, _swapVal2);
	return 1;
}

static int CheckIfNeedSwap(Vector* _v, int _leftVal, int _rightVal)
{
	int isSwapNeeded = 0;
...
static int CheckAndPerformSwap(Vector* _v, int _curIndex, int _mode)
{
	...
	if (!_mode)
	{
		VectorGet(_v, _curIndex + 1, &leftVal);
		VectorGet(_v, _curIndex + 2, &rightVal);
		isSwapNeeded = CheckIfNeedSwap(_v, leftVal, rightVal);
		if (isSwapNeeded)
		{
			// swapOccured = BubbleSwap(_v, _curIndex + 1, _curIndex + 2);
			swapOccured = BubbleSwap(_v, _curIndex + 1, _curIndex + 2, leftVal, rightVal);  
		}
	}
	/* Shake sort mode */
	if (_mode)
	{
		VectorGet(_v, _curIndex, &leftVal);
		VectorGet(_v, _curIndex + 1, &rightVal);
		isSwapNeeded = CheckIfNeedSwap(_v, leftVal, rightVal);
		if (isSwapNeeded)
		{
			// swapOccured = BubbleSwap(_v, _curIndex, _curIndex + 1); 
			swapOccured = BubbleSwap(_v, _curIndex, _curIndex + 1, leftVal, rightVal);
		}
	}
	return swapOccured;
}
