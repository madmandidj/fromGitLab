...
int SortAscend(int _vecValL, int _vecValR)
{
...
int SortDescend(int _vecValL, int _vecValR)
{
...
ADTErr InsertionSort(Vector* _vec, sortFunc* _func)
{
	...
	for (vecIndex = 1; vecIndex = numofitems; ++vecIndex)
	{
		...
		while (curIndex > 1)
		{
			VectorGet(_vec, vecIndex, &vecValL);
			VectorGet(_vec, vecIndex - 1, &vecValR);
			needSwap = _func(vecValL, vecValR);
			if (needSwap)
			{
				SwapVecValsInsertSort(_vec, vecIndex, vecIndex - 1);
			}
			else
			{
				break;
