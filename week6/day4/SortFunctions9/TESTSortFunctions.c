...
static void TestMergeSort10000()
{
    ...
    start_t = clock();
    errResult = MergeSort(vec);
    PrintErrResult("TestMergeSort10000", errResult, ERR_OK);
    end_t = clock();
    total_t = (end_t - start_t);
    printf("Time elapsed for TestMergeSort50000: %f\n", ((float)total_t) / CLOCKS_PER_SEC);
    /* VectorDestroy(vec);    TODO: Why does this cause: Error in `./a.out': double free or corruption (out): 0xb7545008 */ 
}
...
static void TestMergeSort50000()
{
    ...
    start_t = clock();
    errResult = MergeSort(vec);
    PrintErrResult("TestMergeSort50000", errResult, ERR_OK);
    end_t = clock();
    total_t = (end_t - start_t);
    printf("Time elapsed for TestMergeSort50000: %f\n", ((float)total_t) / CLOCKS_PER_SEC);
    /* VectorDestroy(vec);    TODO: Why does this cause: Error in `./a.out': double free or corruption (out): 0xb7545008 */ 
