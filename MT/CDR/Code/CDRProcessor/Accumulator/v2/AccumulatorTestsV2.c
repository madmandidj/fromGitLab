...
UNIT(AccumulatorGetSubscriberAfterMultipleUpdated)
	...
	err = AccumulatorGetSubscriber(accum, &subscriber1, &subFound);
	ASSERT_THAT(err == 1);
	err = AccumulatorGetSubscriber(accum, &subscriber2, &subFound);
	ASSERT_THAT(err == 1);
	err = AccumulatorGetSubscriber(accum, &subscriber3, &subFound);
	ASSERT_THAT(err == 1);
	err = AccumulatorGetSubscriber(accum, &subscriber4, &subFound);
	ASSERT_THAT(err == 0);
