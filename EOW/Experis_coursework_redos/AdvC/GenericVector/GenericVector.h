 ...
 * 
 * @author Author EyalAlon (eyal171@yahoo.com) 
 */
...
/**  
 * @brief Dynamically create a new vector object of given capacity and blockSize  
 ...
 * @param[in/out] _vector - Vector to be deallocated. *_vector = NULL after dealloc.
 ...
 * @retval ERR_NOT_INITIALIZED if (NULL == _vector || NULL == _item).
 * @retval ERR_OVERFLOW if resize needed and block size is zero.
 * @retval ERR_REALLOCATION_FAILED if no memory available.
 */
 ...
 * @param[out] _pValue - pointer to variable that will receive deleted item.
 * @return success or error code. 
 * @retval ERR_OK on success. 
 * @retval ERR_NOT_INITIALIZED if (NULL == _vector || NULL == _pValue).
 * @retval ERR_UNDERFLOW if no items in vector.
 ...
 * @retval ERR_OK on success. 
 * @retval ERR_NOT_INITIALIZED if (NULL == _vector || NULL == _pValue).
 * @retval ERR_WRONG_INDEX if _index == 0 or bigger than num of items.
 * @warning _pValue can't be null. this will be assertion violation.
 ...
 * @return success or error code. 
 * @retval ERR_OK on success. 
 * @retval ERR_NOT_INITIALIZED if (NULL == _vector).
 * @retval ERR_WRONG_INDEX if _index == 0 or bigger than num of items.
 * @warning Index starts from 1.
 */
 ...
 * @param[in] _vector - Vector to use.
 * @return  capacity of vector, 0 if invalid.			
 */
 ...
 * @return number of times the user functions was invoked. 0 if _vector == NULL or if _action == NULL.
 */
