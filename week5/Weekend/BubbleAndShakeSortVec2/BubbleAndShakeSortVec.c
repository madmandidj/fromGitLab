...
static int SwapVecVals(Vector* _v, int _index1, int _index2)
{
	int swapVal1;
	int swapVal2;
	/* index + 1 because vector starts from index = 1, not index = 0 */
...
ADTErr ShakeSort(Vector* _v)
{
	...
	else
	{
		...
		while (!isSorted)
		{
			/* Shake sort to the right */
			swapOccured = 0;
			for (shakeIndexR = 0; shakeIndexR < shakeLength - 1; ++shakeIndexR)
			{
			...
			/* Shake sort to the left */
			for (shakeIndexL = shakeIndexR; shakeIndexL > vecItemsNum - shakeLength - 1; --shakeIndexL)
			{
