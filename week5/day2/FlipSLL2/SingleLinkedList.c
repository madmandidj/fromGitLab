/*
Filename: 		SingleLinkedList.c 
Description:	Implementation for SLL API functions: 
				ListInsertHead, ListRemoveHead, ListInsertByKey, ListRemoveByKey,
				ListInsertByKeyRec, ListRemoveByKeyRec, PrintList
Created: 		13/07/17
Created by:		Eyal Alon
Last modified: 	17/07/17
Modified by:	Eyal Alon
TODO: 
- Make corrections that are in comments
*/
#include <stdio.h>
...
{
	/* TODO:  rearrange order of this function */
	if (NULL == _head)
	{
		*_item = NULL;
		return NULL;
...
static Person* LGTR(Person* _head)
{
...
Person* ListGetTailRec(Person* _head)
{
	if (NULL == _head)
	{
		/*
...
Person* FlipSLLRec(Person* _head)
{
	Person* tempHead;
	Person* returnHead;
	if (_head->m_next != NULL)
	{
		tempHead = FlipSLLRec(_head->m_next);
		returnHead = tempHead;
		while(NULL != tempHead->m_next)
		{
			tempHead = tempHead->m_next;
		}
		tempHead->m_next = _head;
		tempHead->m_next->m_next = NULL;
		return returnHead;
	}
	else
	{
