...
/*
TODO:
* Do Radix using the Counting sort function
* Make the use of index / index + 1 with Vector API uniform for all functions
* Make the counting sort receive a pointer to function that determines the sort type
* Make Quick Sort Rec and Iter work
...
static void FillAndProcessRangeArray(Vector* _vec, int* _rangeArr, int _maxRange, int _numOfItems)
{
	size_t index;
	int vecVal;
...
static void FillSortedArray(Vector* _vec, int* _rangeArr, int* _sortedArr, int _numOfItems)
{
	size_t origIndex;
	int vecVal;
...
ADTErr CountingSort(Vector* _vec, int _maxValue)
{
	...
	FillAndProcessRangeArray(_vec, rangeArr, _maxValue, numOfItems);
...
static void ExtractCurDigit(Vector* _vec, int* digArr, int _numOfItems, int _nDigits, int _curDigit, int _radix)
{
	size_t index;
	int vecVal;
	int digitIndex = 0;
